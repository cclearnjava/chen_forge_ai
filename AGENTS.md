...
<!--
  ...
  This file is the primary instruction source for Codex sessions.
  Edit freely — the installer will NOT overwrite an existing AGENTS.md.
-->
...
This project is managed by [wow-harness](https://github.com/anthropics/wow-harness) — an AI agent session governance framework. The following hooks and guards run automatically during Codex sessions.
...
- Co-Authored-By header: `Co-Authored-By: Codex Opus 4.6 <noreply@anthropic.com>`

### Code Quality
- All Python commands use `python3`, never bare `python`
- Subagent model: review/audit/evaluator agents must use `Codex-opus-4-6`
...
Path-scoped rules in `.Codex/rules/*.md` auto-load when Codex reads files matching their trigger patterns. No manual loading needed.

当前 path-scoped rules：

- `.Codex/rules/backend-routes.md`：后端 API、路由、FastAPI 文件相关规则。
- `.Codex/rules/env-vars.md`：环境变量、密钥、`.env` 相关规则。
- `.Codex/rules/repo-structure.md`：项目结构、目录边界、文件放置规则。
- `.Codex/rules/source-of-truth.md`：真相来源和文档优先级规则。
- `.Codex/rules/review-agent-isolation.md`：审查 agent 隔离规则。
- `.Codex/rules/closure-semantics.md`：完成语义和关闭条件规则。
