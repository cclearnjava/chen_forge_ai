...
1. 先运行：
   ```bash
   python3 .Codex/skills/harness-dev-handoff/scripts/collect_handoff_context.py
   ```
...
如果 PostToolUse hook 正常工作，接手者编辑文件时会自动收到该领域的上下文。如果没收到，检查 `.Codex/settings.json` 中的 hook 配置。
...
```bash
python3 .Codex/skills/harness-dev-handoff/scripts/collect_handoff_context.py \
  --cloud-root ~/.Codex/projects/-Users-nature------{{PROJECT_NAME}} \
  --recent-sessions 3 \
  --recent-messages 3
```
